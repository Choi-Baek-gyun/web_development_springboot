...
import me.qorrbs.springbootdeveloper.domain.Article;
import me.qorrbs.springbootdeveloper.dto.AddArticleRequest;
import me.qorrbs.springbootdeveloper.repository.BlogRepository;
...
class BlogApiControllerTest {
    ...
    /*
        ...
        : 이 클래스로 만든 자바 객체를 JSON 데이터로 변환하는 것 -> 직렬화
            역으로 JSON 데이털ㄹ 자바 객체로 변환하는 것 -> 역직렬화

            직렬화(Serialization) - 자바 객체 -> JSON
            역직렬화(Deserialization) - JSON -> 자바객체
            ...
            형식에 맞게 변환하는 과정이 필요합니다. 이 작업을 직렬화/역직렬화
                ...
                1) 직렬화 Java 시스템 내부에서 사용되는 객체를 외부에서 사용하도록
                    변환하는 과정을 뜻하는데,
                    예를 들어 title은 "제목", content는 "내용"이라는 값이 들어가있는
                    개체가 있다고 가정했을 때,
                    자바 상에서는

                    @AllArgsConstructor
                    public class Article {           클래스, 생성자, 필드 생성
            ...
            When - 블로그 글 추가 API에 요청을 보냅니다. 요청 타입은 JSON이며, given절에서
                미리 만들어 둔 객체를 요청 본문으로 함께 보냅니다.
            Then - 응답 코드가 201 Created인지 확인할 겁니다.
                Blog를 조회했을 때 전체 크기가 1인지 확인할 겁니다.
                실제 저장된 데이터와 요청 값을 비교
     */
    ...
    public void addArticle() throws Exception {
        ...
        List<Article> articles = blogRepository.findAll();  // repositorqy에서 전체 글(객체)을 가져와 list에 답았습니다.

        assertThat(articles.size()).isEqualTo(1);  // list 내에 element개수 확인해서 그게 1이면 true

        // list인 articles를 확인해서 가장 첫번째인(0번) 객체를 꺼내서, title 확인했더니 String "title"인지 확인
        ...
        /*
            ...
            그 이후에 MockMvc를 사용해 HTTP 메서드, URL, 요청 본문, 요청 타입 등을 설정 뒤 테스트 요청을 보냈습니다.
    ...
    public void deleteArticle() throws Exception {
        ...
        // then - 검증 단계에서 블로그 글 목록을 전부 다 가지고 올 겁니다. -> size()체크 했더니 0이거나 / .isEmpty() 메더스 활용
        // 전부 다 가지고 오는 단계
        List<Article> articles = blogRepository.findAll();

        // 가지고 온 List에 아무런 값이 없는지 확인하는 단계 -> 왜 아무 것도 없냐?
        // Test 메서드는 메서드 단위로 실행된다고 합니다.
        // given 단계에서 생성한 객체 하나만 있고, 그것을 삭제했기 때문에
