...
/*
    1. 사전 지식 : API와 REST API
        ...
        식당으로 알아보는 API
            여러분이 손님인데 식당에 들어갔어요 -> 점원에게 요리를 주문합니다(주방으로 달려가서 요리를 주문하는게 아니라.)
            그리고 점원은 주방에 가서 요리를 만들어 달라고 합니다.
                 요 청       요 청
            손님 <----> 점원 <----> 주방
                 응 답       응 답
            위 형태를 띄고 있는데, 여기서 손님은 클라이언트(client), 주방에서 일하는 요리사를 서버(server)라고 생각하시면 됩니다.
            ...
            웹의 장점을 최대한 활용하는 형태라고 알려진 REST API
                ...
                단점 :
                     HTTP 메서드, 즉 GET, POST와 같은 방식의 새구에 제한이 있음.
                     설계를 위해 공식적으로 제공되는 표준 규약이 있음.
                ...
                REST API를 사용하는 방법
                    규칙 1. URL에는 동사를 쓰지 말고, 자원을 표시한다.
                        *자원 : 가져오는 데이터를 의미. 에를 들어 학생 중에 ID가 1인 학생의 정보를 가져오는 URL은
                        1)예시 : /student/1
                        2)에시 : /get-student?student_id=1
                    과 같은 방식으로 설계할 수 있는데,

                    이 중 더 REST API에 맞는 형식은 1)예시에 해당함.
                    2)예시의 경우 자원이 아닌 다른 표현을 섞어 사용했기 때문(GET)

                    동사를 사용해서 생기게 되는 추후의 문제점 예시 -> 데이터를 요청하는 URL을 설계할 때
                        A개발자는 get, B개발자는 show를 쓰면 get-student, show-data 등으로
                        협의가 이루어지지 않은 설계가 될 가능성이 있다.
                    ...
                    규칙 2. 동사는 HTTP 메서드로
                        ...
                        예를 들어 블로그에 글을 쓰는 설계를 한다고 가정했을 때
                            ...
                            1)ID가 1인 블로그 글을 '조회'하는 API : GET/article/1
                            2)블로그 글을 '추가'하는 API         : POST/articles         -> 추가가 완료되기 전이라 DB상 ID가
                            3)블로그 글을 '수정'하는 API         : UPDATE/articles/1
                            4)블로그 글을 '삭제'하는 API         : DELETE/articles/1

                        *GET / POST 등은 URL에 입력하는 값이 아니라 내부적으로 처리하는 방식을 미리 정하는 것으로
                        실제로 HTTP 매서드는 내부에서 서로 다른 함수로 처리하는데, 대놓고 적는 일은 잘 없습니다.
