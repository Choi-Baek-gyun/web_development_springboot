    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        트큰 기반 인증 :
            *토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            서버가 토큰을 생성해서 클라이언ㄴ트에게 제공하면, 클라이언트는 이 토큰을 가지고 있다가
            여러 요청을 해당 토큰과 함께 신청함. 이후 서버는 토큰을 확인하여 유효한 사용자인지를 검증하게 됨.
            ...
            토큰을 전달하고 인증 받는 과정
                ...
                5) 서버는 토큰이 유효한지 검증
                6) 토큰이 유요하다면 클라이언트의 요청한 내용을 처리

            토큰 기반 인증의 특징
                1) 무상태성
                    사용자의 인증 정보가 담겨 있는 토큰이 서버가 아닌 클라이언트에 있으므로
                        서버에 저장할 필요 x
                    서버가 데이터를 유지하고 있으려면 그만큼 자원을 소비해야 하는데,
                    토큰 기반 인증에서는 클라이언트에서 인증 정보가 담긴 토큰을 생성하고 인증하여
                    서버 입장에서는 클라이언트의 인증 정보를 저장하거나 유지할 필요 x
            ...
            JWT(JSON Web Token) -> JSON 형태로 주고받는 거죠
                ...
                JWT 구조 : 온점(.)을 기준으로 헤더(Header), 내용(Payload), 서염(signature)로 이루어짐.
                    ...
                    2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)
                        ...
                        2. JWT 예시
                        {
                            "iss":"qorrbs9341@gmail.com", // 등록된 클레임
                            "iat": 1622370678,
                            "exp":16222372678,
                            "https://qorrbs9341@com/jwt_claims/is_admin": true // 공개 클레임
                        }

                    iss, iat, exp는 JWT 자체에서 등록된 클레임
                    URI로 네이밍된 "https://qorrbs9341@com/jwt_claims/is_admin": 공개 클레임
                    등록 / 공개 클레임 제외 나머지는 전부 비공개 클레임에 해당

                    3) 서명 : 해당 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도로 사용.
                        헤더의 인코딩 값과 내용의 인코딩 값을 합친 후에 비밀키를 사옹해 해시값을 생성
                    ...
                    2. JWT 서비스 구현
                        ...
                        build.gradle로 이동해서 의존성 추가할 겁니다.

                        토큰 제공자를 추가할 겁니다.
                            jwt를 사용해서 JWT를 생성하고 유효한 토큰인지 검증하는 클래스 추가할 예정
                                jwt 토큰을 만들려면 이슈 발급자(issuer), 비밀키(secret_key)를 필수로
                                apllication.yml로 이동합니다.
