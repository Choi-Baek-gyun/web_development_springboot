...
public class TokenProvider {
    ...
    public String generateToken(User user, Duration expiredAt) {
        Date now = new Date();
        // 빨간 줄이 뜨는 상태는 아직 정의하지 않았기 때문입니다. -> 바로 밑에 정의할 예정 makeToken
    ...
    private String makeToken(Date expiry, User user) {
        ...
        return Jwts.builder()
                ...
                // 내용 iss : maybeags@gmail.com(JwtProperites에서 설정했습니다.)
    ...
    public Authentication getAuthentication(String token) {
        //getClaims 최초 상황 시에 아직 정의 안했습니다.
...
/*
    1. makeToken() :
        ...
        set 계열 메서드를 통해 여러 값을 지정했는데
        header에는 typ, 내용에는 iss, iat, exp, sub
        claim에는 유저 id 지정
        토큰을 만들 때 properties 파일에 선언해둔 비밀값과 함께 HS256 방식으로 암호화

    2. validToken() :
        토큰이 유요한지 검증하는 메서드. 프로퍼티즈 파일에 선언한 비밀값과 함께 토큰 복호화
    ...
    3. getAuthentication() :
        ...
        이떼, UserPasswordAuthenticationToken의 첫 argument로 들어가는 User는
    ...
    4. getUserId() :
        ...
        토큰을 복호화 한 다음 클레임이 가져오는 private getClaims()를 호출.
        클레임 정보를 반환받고, 클레임 id키로 저장된 값을 가져와 반환.

        다음 단계 -> 코드가 제대로 동작하는지 확인하기 위해서 테스트 코드 작성 예정
            20250116에 하겠습니다.
 */
