...
/*
    ...
    4) create test 선택
    5) 저희 프로젝트 상으로는 JUnit5로 고정돼있습니다.
 */
...
class TestControllerTest {
    ...
    public void mockMvcSetup(){
        this.mockMvc = MockMvcBuilders // builder().build()의 응용 형태
...
/*
    (1) : perform() 메서드는 요청을전송하는 역할을 하는 메서드.
    ...
    (4) : jsonPath("$[0].{필드명}")은 JSON 응답값의 값(value)을 가져오는 역할을 하는 메서드.
        0(인데스)번째 배열에 들어 있는 객체의 id, name의 값을 가져오고 저장된 값과 같은지 확인
        (memberRepository.savedMember.getId()등을 이용해서)


            다 하신 분들은
                테스트 코드 패턴을 연습하기 위해서
                src-main-java-me.qorrbs-springbootdeveloper 폴더에
                QuizConteroller.java를 생성하세요.
 */
/*
    @SpringBootTest :
