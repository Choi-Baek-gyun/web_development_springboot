...
-- 예를 들어 실습 데이터에서 회원 수를 구하려면 어떡해야핳까, 전체 회원 수가 아니라,
-- 국가별 회원 수를 구하는 방법, 제품별 매출을 구하는 방법 등을 실제로 알아볼 수 있음.

-- SQL내에서 사용하는 함수들은 주 평균, 개수, 합계 등을 구하는 '집계 함수',
...
-- 예를 들어 '국가별 회원 수를 구하라' 혹은 '일별 매출 계산 하라'와 같은
-- 그룹화 기준을 지정해서 원하는 계산을 수행할 수 있음.

-- GROUP BY로 계산한 결과를 필터링하는 HAVING 명령어
-- GROUP BY를 적용해서 '국가별 회원 수를 구하고' 이후에 '그리고 회원 수가 10명 이상인' '국가만 도출하라'는 등
-- 집계 함수로 계산한 결과 중 조건에 맞는 데이터만 필요한 경우에는 WHERE을 쓰는 것이 아니라 'HAVING'을 사용합니다.

-- 집계 함수 예시
-- 예제 : users에서 모든 행 수를 세어봅니다. : COUNT
SELECT count(*) 		-- COUNT(컬럼 명, *) 집계함수에 해당
	FROM users			-- 대상 테이블 전체의 행의 개수를 세는 역할
	;
...
SELECT min(price)
	FROM products
	;
-- products에서 최고가를 구하시오(price 컬럼 이용)
SELECT max(price)
	FROM products
	;
-- products에서 전체 데이터 가격의 합을 구하시오(price 컬럼 이용)
SELECT sum(price)
	FROM products
	;
-- products에서 price의 평균(단, 결과값의 컬럼 명을 avgPrice로 변경)
SELECT round(avg(price), 2) AS avgPrice
	FROM products;
-- ROUND(집계 함수 결과, 소수점 자리수)
...
ROUND(집계 함수 결과, 소수점 자리수) : 소수점 자리를 지정한 자리수까지 반올림하여 반환
SUBSTR(컬럼 명, 시작 위치, 가져올 문자 개수) : 문자열을 지정한 시작 위치부터 지정한 문자 개수만큼 반환
LENGTH(컬럼 명) : 문자열의 길이 반환
UPPER(컬럼 명) : 알파벳 문자열을 대문자로 반환
LOWER(컬럼 명) : 알파벳 문자열을 소문자로 반환

집계 함수는 여러 행의 데이터를 '하나의 결과 값'으로 집계하는 반면, 일반 함수는 한 행의 데이터에 하나의 결과값을 반환한다.
이상을 이유로 집계 함수는 SELECT에서만 사용 가능
일반 함수는 SELECT뿐만 아니라 WHERE에서도 사용 가능
이상의 함수 예시들은 전부 다가 아니라 일부이므로 필요 시마다 소개하도록 하겠습니다.
...
GROUP BY 
	...
	-- 그 후, 집계 합수는 특정 컬럼을 기준으로 데이터를 그룹화 한 후에, 그룹별로 집계 함수를 적용해야 합니다.

-- 예제 : users에서 country가 Korea인 회원수를 출력
