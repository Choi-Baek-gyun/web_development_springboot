...
1-1. SQL에서 JOIN은 여러 테이블에서 데이터를 가져와 결합하는 기능을 합니다.
	...
	이상에서 이룽어지는 데이터 분산을 '데이터 정규화(data nomalization)'이라고 하며
	...
	1) JOIN
		...
		실습 데이터를 기준으로.
		ex) 국가별로 주문 건 수를 알아보고 싶다면 -> users와 orders가 필요함.
		즉, 하나의 테이블만으로는 원하는 정보 출력이 불가능합니다.
		
		일반적으로 JOIN의 개념은 중학교 1학년 때 배우는 집합 개념을 떠올리시면 좀 쉽습니다.
		두 개의 데이터 집합을 하나로 결합하는 기능을 지님.
		즉, 기본적으로는 '서로 다른 두 테이블 간의 공통 부분인 키를 활용하여' 테이블을 합칩니다.
	
-- SELECT *
-- FROM users u INNER JOIN orders o
-- ON u.id = o.user_id;
		
SELECT u.id, username, staff_id, order_date
FROM users u INNER JOIN orders o
ON u.id = o.user_id
;
...
-- 회원 정보와 주문 정보를 하나로 결합하기 위해 users와 orders를 INNER JOIN으로 묶고,
-- '후속 조건'으로 "주문 정보가 있는 회원의 정보만 출력하기 위해" u.id = o.user_id를

-- 여기서 여러분들에게 연습 문제 시에만 설명했던 부분을 명확하게 설명하고 넘어가야 합니다.

-- users PK인 id는 회원 id에 해당합니다.
-- orders의 PK인 id는 주문 id에 해당하고 ,2번째 컬럼인 user_id는
-- 