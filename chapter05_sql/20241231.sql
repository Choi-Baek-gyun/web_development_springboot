...
-- 결과 창의 첫 행에 각각 Find, Insight, with SQL을 3개의 칸에 걸쳐
-- 순서대로 출력하는데, 컬럼명을 순서대로 First, Second, Third로 출력
select 
	"Find" as "First",
	"Insight" as "Second",
	"with SQL" as "Third";
...
select *
	from users;
-- users.csv 파일에 있는 모든 테이블과 컬럼과 값이 출력됐음을 확인 가능
-- * : asterisk = all : 와일드카드라는 표시 // java
-- select * from users; : users 테이블에 있는 모든 컬럼의 값을 조회하라.

-- 연습 문제 2 : 제품 정보 테이블 products 에 있는 모든 데이터를 출력하시오.
select *
	from products;
