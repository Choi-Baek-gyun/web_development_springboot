...
-- 현업에서는 데이터의 일부 컬럼을 가져오거나 상위 n개의 데이터를 조회하는 것뿐만 아니라
-- 특정 컬럼의 값이 A인 데이터만 가져오는 등 복잡한 쿼리를 작성할 일이 있습니다.
...
-- select절 + from 테이블 명 + where
-- between : 시작값과 종료값을 '포함'하는 범위 내의 데이터를 조회
--			 시간 값을 조회할 때는 [칼럼 명] BETWEEN [시작 날짜] AND [종료 날짜]
-- 			 마찬가지로 시작 날짜와 종료 날짜를 포함한 모든 것을 출력해준다.

-- 예제 : 가입 일시가 2010-12-01부터 2011-01-01까지인 회원 정보 출력
SELECT *
	FROM users
...
SELECT *
	...
	;
-- LIKE : 해당 전치사 뒤의 작은 따옴표 / 큰 따옴표 내에서는 와일드카드를 사용 간으
--  SQL을 해석하는 컴퓨터가 LIKE 코드를 읽는 순간 와일드카드를 감지하는데
--	SQL에서의 아일드카드는 '%'로 0개 이상의 임의의 문자열을 의미하는
...
SELECT *
	...
	;
-- 거주 국가가 S로 시작하는 ( ex) Spain, Switzerland ) 모두를 출력
...
-- A 컬럼의 값이 B이다 일 때, 특히 NULL 일 때 '=' 대신 사용합니다.
...
SELECT created_at, phone, city, country
	...
	;
	-- 2. orders에서 id가 20 이하이고 price는 30 이상인 제품 정보 출력
		-- 단, 기존 컬럼 전부 출력하고, 정상 가격에서 얼마나 할인되었는지를
		-- discount_amount라는 컬럼 명으로 추출할 것.
SELECT *, 
...
SELECT id, name, price
	...
	WHERE name LIKE "N%"
	;
	-- 5. oreders에서 order_date가 2015-07-01부터 2015-10-31까지가
		-- 아닌 정보만 출력할 것.
SELECT * 
	FROM orders
...
SELECT *
	FROM users
	ORDER BY id ASC			-- ASC : Ascending의 약자 '오름차순'
	;

-- 예제 : users에서 id를 기준으로 내림차순 정렬 후 출력
SELECT *
	FROM users
	ORDER BY id DESC		-- DESC : Descending의 약자 '내름차순'
	;
-- 이상에서 볼 수 있듯이 ORDER BY는 컬럼을 기준으로 행 데이터를 ASC 혹은 DESC로
-- 정렬할 때 사용 : 숫자의 경우는 쉽게 알 수 있지만, 문자의 경우는
-- 아스키 코드(ASCII Code)를 기준으로 합니다.

-- 예제 : users에서 city를 기준으로 내림차순하여 전체 정보를 출력해보세요.
SELECT *
	FROM users
	ORDER BY city DESC
	;

-- 예제 : users에서 created_at를 기준으로 오름차순하여 전체 정보를 출력해보세요.
SELECT *
	FROM users
...
-- ORDER BY의 특징 : 컬럼 명 대신에 컬럼 순서를 기준으로 하여 정렬이 가능.
	-- 컬럼 명을 몰라도 정렬할 수 있다는 장점이 있지만 주의할 필요가 있음.

-- 예제 : users에서 username, phone, city, country, id 컬럼을 순서대로 출력하는데,
-- 		첫 번째 컬럼 기준으로 오름차순 정렬해보세요.
...
-- 컬럼 별로 다양하게 오름차순 혹은 내림차순 적용이 가능

-- ORDER BY 정리
-- 1. 데이터를 가져올 때 지정된 컬럼을 기준으로 한다.
-- 2. 형식 : ORDER BY '컬럼 명' ASC / DESC 
-- 3. 2개 이상의 정렬 기준을 쉼표(,)를 기준으로 합쳐서 지정 가능
	-- 이상의 경우 먼저 지정된 컬럼이 우선하여 정렬됨.
-- 4. 2개 이상의 정렬 기준을 지정할 때 각각 지정 가능
	-- 이상의 경우 각 컬럼 당 명시적으로 ASC 혹은 DESC를 지정해줘야 함.
...
SELECT *
	...
	ORDER BY price DESC
	;
-- 2. orders에서 order_date 기준 최신 순으로 모든 컬럼 출력
SELECT *
	FROM orders
	ORDER BY order_date DESC
	;
-- 3. orderdetails에서 product_id 기준 내림 차순, 같은 제품 id 내에서는 
-- quantity 기준 오름 차순으로 모든 컬럼 출력
SELECT *
	FROM orderdetails
