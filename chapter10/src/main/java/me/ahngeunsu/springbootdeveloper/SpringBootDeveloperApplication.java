...
/*
    사전 지식 : OAuth
        - 제 3의 서비스에 계정 관리를 맡기는 방식. naver / google / kakao 로 로그인 하기와 같은 예시
        ...
        OAuth를 사용하면 인증 서버에서 발급받은 토큰ㅇ을 사용해서 리소스 서버에
            ...
            리소스 오너 정보를 취득하는 네 가지 방법
                ...
                2) 암시적 승인(Implicit Grant Type) :
                    ...
                    별다른 인증 과정을 거치지 않고 액세스 토큰을 제공받는 형식

                3) 리소스 소유자 암호 자격 증명 승인 타입(Resource Owner Password Credentials Type) :
     ...
     1. 권한 요청    |       5. 액세스 토큰으로 데이터에 접근     | 인증 서버에
            ...
            1. 권한 요청 :
                권한 요청은 클라이언트, 즉 스프링 부터 서버가 특정 사용자 데이터에 접근하기 위해 권한 서버,
                즉 카카오나 구글 권한 서버에 요청을 보내는 거라고 볼 수 있습니다. 요청 URI는 권한 서버마다
                ...
                실제 요청에 쓰이는 요청 URI 예시를 통해 주요 파라미터를 소개하자면,
                   ...
                   🍎권한 요청을 위한 파라미터 예
                    ...
                    client_id : 인증 서버가 클라이언트에 할당한 고유 식별자.
                        이 값은 클라이언트 애플리케이션은 OAuth 서비스에 등록할 때 서비스에서 생성하는 값

                redirect_uri : 로그인 성공 시 이동해야하는 URI
                response_type : 클라이언트가 제공받길 원하는 응답 타입. 인증 코드를 받을 때는 code값을
            ...
            2. 데이터 접근용 권한 뿌여
                ...
                로그인 성곤하면 권한 부여 서버는 데이터에 접근할 수 있게 인증 및 권한 부여를 수신
             ...
             3. 인증 코드 제공
              ...
              4. 액세스 코드 응답
                ...
                보통 다음과 같이 /token POST 요청을 보냅니다.
                    ...
                    grant_type : 권한 유형을 확인하는 데 사용. 이 때는 authorization_code로 설정해야 함.
                    권한 서버는 요청 값을 기반으로 유효한 정보인지 확인하고, 유효한 정보라면 액세스 토큰으로 응답합니다.
             ...
             5. 액세스 토큰으로 API 응답 & 반환
                이제 제공받은 액세스 토큰으로 리소스 오너의 정보를 가져올 수 있습니다.
                ...
                리소스 서버는 토큰이 유효한지 검사한 뒤에 응답합니다.
                        ...
                        여기까지가 권한 부여 코드 승인 타입의 흐름입니다. 대부분의 OAuth를 구현한 라이브러리는
                        이 흐름을 바탕으로 코드를 구현하기 때문에 흐름을 이해하고 넘어가는 것이 좋습니다.

             쿠키란?
                사용자가 어떤 웹사이트를 방문했을 때 해당 웹사이트의 서버에서 여러분의 로컬 환경에 저장하는 작은 데이터를 의미함.
    ...
    쿠키 관련 클래스를 구현할 겁니다.
        ...
        springbootdeeloper 패키지 내에 util 패키지를 생성 -> CookieUtil.java 파일 생성
*/