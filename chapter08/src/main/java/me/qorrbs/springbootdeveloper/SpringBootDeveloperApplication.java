        ...
        1. 스프링 시큐리티(Spring Security)
            1) 인증 vs 인가
                ...
                (2) 인가(Authorization) : 사이트의 특정 부분에 접근할 수 있는지 권한을 확인하는 작업
                    예를 들어 관리자느 관리자 페이지에 들어갈 수 있지만 일반 사용자는 관리자 페이지에
                    들어갈 수 없음.
            ...
            3) 필터 기반으로 동작하는 스프링 시큐리티
                스프링 시큐리티의 풀 로그인 과정을 설정한느 건(코드로 작성하느 건) 그렇게 어렵지 않을 건데,
                내보적으로 아까 보여드린 그림처럼 꽤 복잡한 과정을 거치게 될 겁니다.

                모두 외워햐 하는 거 x, 다만 로그인 과정이 어던 흐름으로 이어지는 지를 이해하면 더 잘 활용 가능
                일단은 회원 가입 / 로그인 관련 부분 코딩으로 풀어나갈 예정
